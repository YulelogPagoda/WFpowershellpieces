# This is an example of a firewall GPO build for securing a DNS Server
...
# DNS Control Rules
New-NetFirewallRule -DisplayName "DNSManagementRPC" -Description "DNS Control RPC" -GPOSession $gposession -LocalPort RPC -Service DNS -Program %systemroot%\System32\dns.exe -Protocol TCP | Set-NetFirewallRule -RemoteAddress $managementIPs
New-NetFirewallRule -DisplayName "DNSManagementRPCepmap" -Description "DNS Control RPCepmap " -GPOSession $gposession -LocalPort RPCepmap -Service RPCSS -Program %SystemRoot%\system32\svchost.exe -Protocol TCP | Set-NetFirewallRule -RemoteAddress $managementIPs


# DNS services inbound rules
New-NetFirewallRule -DisplayName "DNSTCP" -Description "DNS Services Inbound TCP" -GPOSession $gposession -LocalPort 53 -Protocol TCP
New-NetFirewallRule -DisplayName "DNSUDP" -Description "DNS Services Inbound UDP" -GPOSession $gposession -LocalPort 53 -Protocol UDP

# Save the entry into the GPO
